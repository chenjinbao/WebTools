...
public class RfaInfoWriter {
    ...
    public static File write(InputStream boardInputStream, RfaExcelConfig excelConfig) {
        ...
        try {
            ...
            files.add(output(updateXml(rrus, "en"), "en"));
            files.add(output(updateXml(rrus, "zh"), "zh"));
    ...
    private static Document updateXml(List<String> rrus, String language) {
        InputStream in = null;
        Document document = null;
        try {
            String rruStr = StringUtils.join(rrus, '/');

            String src = "com/zte/gu/webtools/excel/rfa/template/EDMS_SPM_en.xml";
            if ("zh".equalsIgnoreCase(language)) {
                src = "com/zte/gu/webtools/excel/rfa/template/EDMS_SPM_zh.xml";
            }
